...
func TestHandlers_GetUserById(t *testing.T) {
	...
	tests := []struct {
		...
		fields  fields
		args    args
		want    api.GetUserByIdResponseObject
		wantErr bool
	}{
		{
			name: "happy path",
			...
			want: api.GetUserById200JSONResponse{
				Id:   1,
				Name: "Alice",
			},
			wantErr: false,
		},
		{
			...
			want: api.GetUserById404JSONResponse{
				Code:  404,
				Error: "Not Found",
			},
			wantErr: false,
		},
		{
			...
			want: api.GetUserById500JSONResponse{
				Code:  1,
...
func TestHandlers_CreateUser(t *testing.T) {
	...
	tests := []struct {
		...
		fields  fields
		args    args
		want    api.CreateUserResponseObject
		wantErr bool
	}{
		{
			name: "happy path",
			...
			args: args{
				ctx: context.Background(),
				request: api.CreateUserRequestObject{
					Body: &api.CreateUserJSONRequestBody{
						Name: "Alice",
					},
				},
			},
			want: api.CreateUser201JSONResponse{
				Id: 10,
			},
			wantErr: false,
		},
		{
			...
			want: api.CreateUser400JSONResponse{
				Code:  3,
		...
		},
		{
			...
			want: api.CreateUser500JSONResponse{
				Code:  -1,
				Error: "Internal Server Error",
			},
			wantErr: false,
